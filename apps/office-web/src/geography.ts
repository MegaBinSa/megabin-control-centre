...
import { isOfficeMountCurrent, type OfficeLocation } from "./office-shell.js";
...
export async function renderGeographyWorkspace(
  ...
  adapter.mount(required<HTMLElement>(root, "#map"));
  const load = async () => {
    model = await api.geographyMap<GeographyMapModel>(
      required<HTMLSelectElement>(root, "#region").value
    );
    if (context && !isOfficeMountCurrent(context.mount)) return;
    paint();
  };
  const paint = () => {
    const showAddresses = required<HTMLInputElement>(root, "#addresses").checked;
    const showDepots = required<HTMLInputElement>(root, "#depots").checked;
    adapter.setPolygons(
      model.territories
        .filter(
    ...
    required(root, "#territories").innerHTML =
      model.territories
        .map(
          (item) =>
    ...
    root.querySelectorAll<HTMLButtonElement>("[data-territory]").forEach(
      (button) =>
        (button.onclick = () => {
  ...
  const select = (id: string) => {
    ...
    paint();
    required(root, "#details").innerHTML = selected
      ...
      : "";
    root.querySelector("#edit-territory")?.addEventListener("click", () => openEditor(selected));
  };
  ...
  root.querySelector("#region")?.addEventListener("change", () => void load());
  root.querySelector("#depots")?.addEventListener("change", paint);
  root.querySelector("#addresses")?.addEventListener("change", paint);
  const dialog = required<HTMLDialogElement>(root, "#territory-editor");
  const form = required<HTMLFormElement>(root, "#territory-form");
  const showDepot = (id: string) => {
    const depot = model.depots.find((item) => item.depotId === id);
    if (!depot) return;
    required(root, "#details").innerHTML =
  ...
  root.querySelector("#create-territory")?.addEventListener("click", () => openEditor());
  root.querySelector("#cancel-edit")?.addEventListener("click", () => {
    adapter.deleteDraft();
    dialog.close();
  });
  root.querySelector("#delete-draft")?.addEventListener("click", () => {
    (form.elements.namedItem("geometry") as HTMLTextAreaElement).value = "";
    adapter.deleteDraft();
  });
  const body = () => ({
    serviceRegionId: required<HTMLSelectElement>(root, "#region").value,
  ...
  root.querySelector("#preview")?.addEventListener("click", async () => {
    ...
    if (!id) {
      required(root, "#impact").textContent =
    ...
    required(root, "#impact").textContent =
      `${result.length} active service(s) require review if saved.`;
  });
  ...
  const reviewDialog = required<HTMLDialogElement>(root, "#review-dialog");
  root.querySelector("#reviews")?.addEventListener("click", async () => {
    const reviews = await api.geographyReviews<readonly Record<string, unknown>[]>(
      required<HTMLSelectElement>(root, "#region").value
    );
    required(root, "#review-items").innerHTML =
      reviews
        .map(
          (review) =>
  ...
  root.querySelector("#close-reviews")?.addEventListener("click", () => reviewDialog.close());
  await load();
}
