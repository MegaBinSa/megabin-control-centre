...
Website → authenticated receipt → immutable submission → bounded asynchronous processing → deterministic matches and suggestions → Office review → frozen approval → transactional activation.
...
Validation failures remain `invalid`, business uncertainty becomes `needs_review`, and technical failures become `failed`. Approval and rejection are never technical retries. Events contain identifiers and safe lifecycle metadata, never contact or address payloads.
