...
Activation is authorised against the frozen `serviceRegionId`. A region-scoped actor must hold both `website_intake.activate` and regional `master_data.write`; the transaction then uses private master-data owner primitives to create the Client, Contact, Address, Service and Configuration with their normal audit and event facts. Those primitives are not browser/RPC endpoints. Ordinary unscoped master-data creation remains global-only, and a frozen region that differs from the processed intake region is rejected before any write.
