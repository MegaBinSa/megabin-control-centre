...
| CFG-BACKUP | PostgreSQL | One-hour target RPO, four-hour target RTO, Shaun/Sidney authorities and isolated target configured; logical restore passed in 296 seconds; component rollback/current restoration passed; PITR disabled; RPO unmet | Obtain Sidney confirmation; rehearse forward repair; add retained hourly recovery points or approve pilot exception; decide durable 12-month archive and production backup design | `staging-recovery`, repository config, Supabase/org operations | Business/Technical | Pilot/Production |
