...
| CFG-BACKUP | PostgreSQL | Explicit unresolved register; tooling blocks without RPO/RTO/authority/isolated target | Plan, backup/PITR capability, approved objectives, authorities and restore schedule | Supabase/org operations | Business/Technical | Restore rehearsal/Pilot |
...
| CFG-OBS | Observability | Stable alerts and GitHub evidence artifacts; owner/destination deliberately unassigned | Monitoring/escalation owners, recipients, destination, routing and acknowledgement policy | GitHub/Supabase/monitoring config | Technical/Operations | Operational Internal UAT |
