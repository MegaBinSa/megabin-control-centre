...
| CFG-BACKUP | PostgreSQL | One-hour target RPO, four-hour target RTO, Shaun/Sidney authorities and isolated target configured; logical restore, component rollback and isolated forward repair passed; Environment protection and authority/verifier logins proven; PITR disabled; RPO unmet | Obtain independent post-run confirmations; address hourly recovery points and the 12-month evidence archive | `staging-recovery`, repository config, Supabase/org operations | Business/Technical | Pilot/Production |
