...
| CFG-BACKUP | PostgreSQL | One-hour target RPO, four-hour target RTO, Shaun/Sidney authorities and isolated target configured; Free-plan logical dump only; PITR disabled; RPO unmet | Execute isolated rehearsal; add secure retained hourly recovery points or approve pilot exception; production backup design later | `staging-recovery`, repository config, Supabase/org operations | Business/Technical | Pilot/Production |
