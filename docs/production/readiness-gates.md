...
| Synthetic Internal UAT Passed | Not Run | Catalogue validation only; no aggregate pass evidence | `UAT-OFF-001` and `UAT-DRV-001` Passed manually; execute/sign off the remaining four journeys with release-bound evidence |
...
- **Passed journeys:** `UAT-OFF-001` Office planning and operational observation; `UAT-DRV-001` Driver execution/offline synchronization and authorization isolation.
- **Not Run:** the other four synthetic UAT journeys; the aggregate UAT Passed gate is therefore not achieved.
