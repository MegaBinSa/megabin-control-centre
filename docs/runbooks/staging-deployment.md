...
8. Verify health `buildId`/deployment ID, hosted gateway preflight behavior, allowed actual-request CORS, unknown-origin actual-request denial, Office/Driver authorization boundaries, fake-provider posture, communication capture mode and website intake idempotency. The region-scoped Office persona must be denied global accounting and communications provider-health endpoints; provider posture is independently enforced by the protected environment contract.
