...
3. Review configuration names/status with `pnpm env:validate:staging -- --deployment`; never paste values into logs.
   `MEGABIN_WEBSITE_ONBOARDING_INTEGRATION_KEY` must be the seeded, enabled synthetic identity `megabin-website-onboarding-staging`; a different key fails validation before deployment.
...
8. Verify health `buildId`/deployment ID, hosted browser-equivalent preflights, exact approved-origin headers on authenticated Office profile, a Clients read explicitly scoped to Pretoria Test Region, and Driver bootstrap responses, unknown-origin actual-request denial, Office/Driver authorization boundaries, fake-provider posture, communication capture mode and website intake idempotency. Browser-equivalent preflights must include the actual `authorization`, `content-type` and `x-correlation-id` request headers; accepting a generic `204` without validating allowed headers is insufficient. Wildcard origins are forbidden. A successful profile alone is not Office readiness: the scoped Clients read must return `200` with the approved-origin CORS header. The region-scoped Office persona must be denied global accounting and communications provider-health endpoints; provider posture is independently enforced by the protected environment contract.
