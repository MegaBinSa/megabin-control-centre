...
8. Verify health `buildId`/deployment ID, hosted browser-equivalent preflights, exact approved-origin headers on authenticated Office profile and Driver bootstrap responses, unknown-origin actual-request denial, Office/Driver authorization boundaries, fake-provider posture, communication capture mode and website intake idempotency. Browser-equivalent preflights must include the actual `authorization`, `content-type` and `x-correlation-id` request headers; accepting a generic `204` without validating allowed headers is insufficient. Wildcard origins are forbidden. The region-scoped Office persona must be denied global accounting and communications provider-health endpoints; provider posture is independently enforced by the protected environment contract.
