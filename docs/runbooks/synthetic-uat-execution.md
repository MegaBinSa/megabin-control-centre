...
Before Website Intake activation, retain the approved decision, optimistic version and actor region as evidence. Activation must succeed for a region-scoped Office actor only when the frozen region equals the processed intake region and the actor holds `website_intake.activate` plus regional `master_data.write`. Verify the Client, Contact, Address, Service and Configuration appear atomically with activation audit/event facts. A denied or failed attempt must leave the intake approved and create no partial master data. Never add global scope merely to continue UAT.

For close-out, reconcile the one intake identity across the initial response, durable job/history, identical-submission retry, Office queue, approval and activation. Verify version 4, unchanged frozen approval, one linked Client/Contact/Address/Service/Configuration set, three expected external references, one approval and activation audit/event pair, healthy processing evidence, and no service-linked route, occurrence, financial, communications or SKIP records. Preserve an original Blocked or Failed checkpoint alongside its remediation and successful retest. The catalogue's final retry means the byte-identical Website submission retry; do not manufacture a second activation after the UI has reached `activated`.

## Protected Client SKIP inbound submission

`UAT-SKP-001` starts through the manual-only **Submit staging SKIP UAT inbound** workflow and protected GitHub `staging` Environment. The operator supplies the exact current `main` SHA, reserved provider identity `uat:skip:UAT-SKP-001:20260831:01`, and confirmation `SUBMIT-UAT-SKP-001:xniweqdmswzljcgkfglx:uat:skip:UAT-SKP-001:20260831:01:<source-sha>`.

The request is immutable repository configuration in `config/synthetic-uat-skip-inbound.json`. It authenticates the synthetic Office persona to satisfy the hosted Function gateway, then sends one signed fake-provider request through the real communications inbound boundary. Credentials and the webhook secret remain protected Environment secrets. Evidence excludes sender, command body, credentials, bearer token and signing material. The workflow cannot accept a URL, project, message body or SQL and performs no deployment, migration, seed, reset or direct database operation.

Before dispatch, prove read-only that the fixed sender resolves to one active synthetic contact and service, the reserved provider identity is absent, and the 2026-08-31 Pretoria occurrence has no prior SKIP or exclusion. Prepare the date only through existing Office workflows: lock the roster, generate a route containing the target Client Service and retain its stop identity, then publish the reviewed route version. Do not accept or start its Route Operation merely to manufacture an active-operation conflict. Record source route/version/operation state before injection.

Receipt retention is immutable by provider/message identity. An exact repeated dispatch returns the existing inbound message and SKIP Request without duplicating communications events or operational action; a changed payload under the reserved identity fails closed. The runtime synchronously consumes recognized `SKIP` through `client_skip_consume_inbound`, so success proves a retained inbound and qualified request rather than only raw receipt. Stop if region, service, occurrence, cutoff or route impact differs from prepared evidence.

Continue in Office with explicit approval. Approval creates one active `client_requested_skip` occurrence exclusion and capture-only acknowledgement intent; it must not mutate the Published route or any accepted/in-progress operation. Request replan through the existing SKIP action, verify the Published source remains immutable and the candidate is a new Draft without the excluded stop, and do not auto-publish. Retain concise communications, qualification, approval, exclusion and replan audit/event/correlation evidence without phone numbers or message content.
