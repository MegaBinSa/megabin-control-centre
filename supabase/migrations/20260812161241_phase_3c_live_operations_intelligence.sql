...
create or replace function api.operational_intelligence_record_failure(p_actor_id uuid,p_region_id uuid,p_vehicle_id uuid,p_route_operation_id uuid,p_failure_code text)
returns void language plpgsql security definer set search_path='' as $$ begin
 perform app_private.require_intelligence_access(p_actor_id,'operational_intelligence.process',p_region_id);
 if not exists(select 1 from app_private.vehicles where vehicle_id=p_vehicle_id and service_region_id=p_region_id) then raise exception 'failure_vehicle_mismatch' using errcode='22023'; end if;
 insert into app_private.intelligence_processing_checkpoints(vehicle_id,route_operation_id,rule_version,last_failure_code) values(p_vehicle_id,p_route_operation_id,'phase-3c-v1',left(coalesce(p_failure_code,'processing_failed'),100)) on conflict(vehicle_id) do update set route_operation_id=excluded.route_operation_id,processed_at=now(),last_failure_code=excluded.last_failure_code;
end $$;
...
grant execute on function api.operational_intelligence_apply(uuid,uuid,jsonb,jsonb,uuid),api.operational_intelligence_record_failure(uuid,uuid,uuid,uuid,text),api.operational_facts_list(uuid,uuid,text),api.needs_attention_list(uuid,uuid,text),api.operational_fact_review(uuid,uuid,text,text,uuid),api.live_operations_overview(uuid,uuid),api.operational_fact_detail(uuid,uuid),api.live_operations_vehicle_detail(uuid,uuid),api.live_route_progress_detail(uuid,uuid),api.needs_attention_detail(uuid,uuid) to service_role;
